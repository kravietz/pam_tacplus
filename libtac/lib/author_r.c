...
int tac_author_read_timeout(int fd, struct areply *re, unsigned long timeout) {
    ...
    if (len_from_header > TAC_PLUS_MAX_PACKET_SIZE) {
        TACSYSLOG(
                LOG_ERR, "%s: length declared in the packet %zu exceeds max packet size %d", __FUNCTION__,
    ...
    tb_byte_p = (char *) tb;
    do {
        if (tac_readtimeout_enable && tac_read_wait(fd, timeleft, len_from_header, &timeleft) < 0) {
        ...
        if (packet_read < (ssize_t) len_from_header) {
            TACDEBUG(
                    LOG_DEBUG, "%s: read bytes %zu to %zu of response body", __FUNCTION__, tb_bytes_read,
                    tb_bytes_read + packet_read - 1);
        }
     ...
     */
    len_from_body = TAC_AUTHOR_REPLY_FIXED_FIELDS_SIZE + tb->msg_len + tb->data_len;
    ...
    for (r = 0; r < tb->arg_cnt; r++) {
        if (len_from_body > tb_bytes_read || ((void *) pktp - (void *) tb) > (ssize_t) tb_bytes_read) {
            TACSYSLOG(
    ...
    if (tb->msg_len) {
        ...
        memcpy(msg,
               (unsigned char *) tb + TAC_AUTHOR_REPLY_FIXED_FIELDS_SIZE + (tb->arg_cnt) * sizeof(unsigned char),
               tb->msg_len);
    ...
    switch (tb->status) {
        ...
        case TAC_PLUS_AUTHOR_STATUS_PASS_ADD: {
            ...
            for (r = 0; r < tb->arg_cnt; r++) {
                ...
                if (sep == NULL) {
                    TACSYSLOG(
                            LOG_WARNING, "%s: AUTHOR_STATUS_PASS_ADD/REPL: attribute does not contain a separator: %s",
                            __FUNCTION__, buff);
