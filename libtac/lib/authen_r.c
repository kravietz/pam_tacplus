...
int tac_authen_read_timeout(int fd, struct areply *re, unsigned long timeout)
{
	...
	if (tac_readtimeout_enable && tac_read_wait(fd, timeleft, len_from_header, NULL) < 0)
	{
		TACSYSLOG(
			LOG_ERR, "%s: reply timeout after %lu secs", __FUNCTION__, timeout);
	...
	len_from_body = sizeof(tb->status) + sizeof(tb->flags) + sizeof(tb->msg_len) + sizeof(tb->data_len) + tb->msg_len + tb->data_len;
